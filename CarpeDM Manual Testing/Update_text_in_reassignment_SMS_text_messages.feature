...
Scenario: (#1) The message that is sent to the matchmaker after a rootadmin, admin, or matchmaker reassigns an interview should be updated by a new message
  ...
  And a Braze event arrives at the user of type matchmaker_update
  And an email arrives to the user of the Registration Web Portal with the details of the new interview
  
Scenario: (#2) The message that is sent to the matchmaker after a root admin, admin, or matchmaker reassigns an interview with their synced Google account should be updated with a new message
