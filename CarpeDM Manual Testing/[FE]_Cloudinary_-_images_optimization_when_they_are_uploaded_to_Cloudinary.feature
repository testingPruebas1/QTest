Feature: [FE] Cloudinary - images optimization when they are uploaded to Cloudinary
    ...
    Scenario: (#2) Admin upload a new picture

        Given Admin have logged in
        When Admin upload, edit, rotate, etc; an image

        Then Admin must see that the flow is working correctly (AC#1)

        Then Admin must see that in Cloudinary, the file weight is reduced and the sizewill be also fixed (max 1000 px). (AC#2)

        Then images must be displayed ok in the admin portal, with  good quality and not bluered (AC#3)

        Then Admin must see that in Cloudinary, images from QA are in different folders than Production. (AC#4)
